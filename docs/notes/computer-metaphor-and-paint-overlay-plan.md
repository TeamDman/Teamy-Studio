...
- Current focus:
  - Use the microphone timeline, shared-memory slot pool, named-pipe Python handoff, and daemon dashboard as the first real proving ground for persistent daemon inference.
- Remaining work:
...
- Next step:
  - Continue in `docs/notes/audio-input-inbox-plan.md` with a longer-lived Python daemon loop before returning to paint chrome.
