...
- Remaining work:
  ...
  - Move `audio transcribe --demo <count>` from per-worker model copies to the same one-model batched decoder path used by long-input chunks, after designing a small batch scheduler for multiple short prepared inputs.
