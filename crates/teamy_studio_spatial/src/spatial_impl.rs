...
sguaba::system!(pub struct ScreenSpace using right-handed XYZ);
sguaba::system!(pub struct ClientSpace using right-handed XYZ);
    ...
    Space: CoordinateSystem<Convention = RightHandedXyzLike>,
{
    ...
    #[must_use]
    pub fn from_win32_point(point: POINT) -> Self {
        Self::new(point.x, point.y)
    }

    pub fn to_win32_point(self) -> eyre::Result<POINT> {
        Ok(POINT {
    ...
    #[must_use]
    pub fn coordinate(self) -> Coordinate<Space> {
        self.coordinate
    }
}

pub type ScreenPoint = Point<ScreenSpace>;
pub type ClientPoint = Point<ClientSpace>;

impl ScreenPoint {
    pub fn pack_lparam(self) -> eyre::Result<isize> {
...
impl<Space> Rect<Space> {
    ...
    #[must_use]
    pub fn from_win32_rect(rect: RECT) -> Self {
        Self::new(rect.left, rect.top, rect.right, rect.bottom)
    }

    #[must_use]
    pub fn to_win32_rect(self) -> RECT {
        RECT {
            left: self.left,
            top: self.top,
            right: self.right,
...
pub type ClientRect = Rect<ClientSpace>;
...
pub struct ScreenToClientTransform {
...
impl ScreenToClientTransform {
    ...
    #[must_use]
    pub fn screen_to_client(self, point: ScreenPoint) -> ClientPoint {
        ClientPoint {
    ...
    #[must_use]
    pub fn client_to_screen(self, point: ClientPoint) -> ScreenPoint {
        ScreenPoint {
...
#[must_use]
pub fn drag_threshold_exceeded(
    origin: ClientPoint,
    current: ClientPoint,
    threshold_x: i32,
    threshold_y: i32,
...
#[must_use]
pub fn classify_resize_border_hit(
