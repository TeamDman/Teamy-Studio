...
fn select_log_filter(
    ...
    match (config.debug, config.log_filter.as_deref()) {
        ...
        (_, None) => match rust_log {
            ...
            None => Ok(LogFilterSelection::Default(if config.debug {
                LevelFilter::DEBUG
            } else {
                LevelFilter::INFO
            })),
...
pub fn init_logging(config: &LoggingConfig) -> eyre::Result<()> {
    ...
    let rust_log = if config.log_filter.is_none() {
    ...
    let json_log_path = resolve_json_log_path(config.log_file.as_deref(), Local::now());
...
mod tests {
    ...
    fn debug_conflicts_with_explicit_log_filter() {
        ...
        let error = select_log_filter(&config, None).expect_err("debug plus log-filter should fail");
        assert!(
            error
                .to_string()
    ...
    fn rust_log_is_used_when_explicit_filter_is_omitted() {
        let selection = select_log_filter(&test_logging_config(), Some("warn"))
    ...
    fn debug_defaults_to_debug_filter_when_no_filter_is_provided() {
        ...
        let selection =
            select_log_filter(&config, None).expect("debug default filter should resolve");
    ...
    fn non_debug_defaults_to_info_filter_when_no_filter_is_provided() {
        let selection = select_log_filter(&test_logging_config(), None)
