#[path = "windows_cursor_info/pixel_size.rs"]
mod pixel_size;
...
pub struct CursorInfoVirtualSession {
...
impl CursorInfoVirtualSession {
    pub fn new(config: CursorInfoConfig) -> eyre::Result<Self> {
        let initial_snapshot = capture_snapshot()?;
    ...
    pub fn cached_display_state(&self) -> Arc<TerminalDisplayState> {
    ...
    pub fn resize(&mut self, layout: TerminalLayout) {
    ...
    pub fn poll_output(&mut self) -> eyre::Result<PollPtyOutputResult> {
        let snapshot = capture_snapshot()?;
    ...
    pub fn handle_char(&mut self, code_unit: u32) -> bool {
    ...
    pub fn handle_key_event(&mut self, vkey: u32, is_release: bool) -> bool {
        if is_release {
            return false;
        }
    ...
    pub fn handle_mouse_wheel(&mut self, scroll_up: bool) -> bool {
        if scroll_up {
            self.state.zoom_in();
        } else {
            self.state.zoom_out();
