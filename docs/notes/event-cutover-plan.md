...
- `StartupSession::pump_to_idle` now includes a named tracing-observation stage alongside registered-trigger and shell-host stages, so observed tracing records are drained through the same deterministic pump contract as other runtime work
...
- The active `Timeline` window visual shell has been deliberately moved away from the audio/document-style layout toward the legacy tracing timeline feel: compact controls, a dedicated ruler band, full-height lane rows, cursor guide, hover-brightened timeline items, beveled span/event clips, and a small status strip for direct-manipulation affordances instead of a dashboard/recent-events panel dominating the view.
- The active `Timeline` camera now keeps a fractional display range during zoom animation, uses legacy-style cubic ease-out, compounds new wheel input from the currently displayed range, and labels the ruler as a global base plus relative increments to reduce clutter while preserving orientation.
- Right-button panning now treats the grabbed timeline point as the anchor that must remain under the cursor, and extreme zoom-out paths must clamp to a representable canonical camera range instead of constructing overflow-prone spans.
- Active drag now keeps a fractional display range instead of rounding every pan step to canonical endpoints, and the shell scene path samples the current cursor position during frame construction to reduce cursor-guide lag.
- Timeline pan/zoom/projection now route through a typed `sguaba`-backed viewport-point transform in the active startup scene so mouse-to-timeline translation, zoom anchors, ruler positions, and item projection all use the same coordinate model.
